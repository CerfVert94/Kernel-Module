...
static int __init pinmuxGPIO(void)
{
 int ret=0;
  ...
  if(!cm_per_gpio) {
    ...
    iowrite32(0x17, cm_per_gpio);// p. 1426 : GPIO MUX to GPIO OUTPUT (Mode 7)
  ...
  ret = gpio_request_one(GPIO_PIN9_14 , GPIOF_OUT_INIT_HIGH,
			 DEVICE_NAME "gpio");
...
static int __init kernmodex_init(void)
{
  ...
  /*A REMPLIR register_chrdev*/
   major = 243;
  if ((ret = register_chrdev(major,"/dev/gpio", &char_fops)) != 0) 
  {
