...
static int  pinmuxPWM(void)  
{
  ...
  if(!cm_per_gpio) {
    ...
    iowrite32(0x06,cm_per_gpio);// p. 1426 : GPIO MUX to PWM (Mode 6) 0b 00110
...
static int initPWM(void)
{
  ...
  if(!cm_per_pwm) {
    ...
    iowrite16(0x80,cm_per_pwm); // Very Important register This start in freezed mode.
  ...
  if(!cm_per_pwm) {
    ...
    sregval = ioread16(cm_per_pwm) ; //enable AQ ActionsC
    iowrite16(0b100100, cm_per_pwm);
